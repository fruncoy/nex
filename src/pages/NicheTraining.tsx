...
export function NicheTraining() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        candidate_id: null, // Remove foreign key constraint issue
  ...
  const handleImportTrainees = async () => {
    ...
    if (!selectedCohortForImport) {
      showToast('Please select a cohort', 'error')
      return
    }

    // Auto-assign course types based on candidate category and proceed directly to import
    const candidatesToImport = candidates.filter(c => selectedCandidates.includes(c.id))
    
    // Validate that all candidates have categories
    for (const candidate of candidatesToImport) {
      if (!candidate.category) {
        showToast(`${candidate.name} doesn't have a category assigned. Please update their profile first.`, 'error')
        return
      }
    }

    // Check if any short course candidates and validate course selection
    const shortCourseCandidates = candidatesToImport.filter(c => c.category === 'Short Course')
    if (shortCourseCandidates.length > 0 && selectedCoursesForImport.length === 0) {
      showToast('Please select at least one course for Short Course candidates', 'error')
      return
    }

    try {
      const trainingRecords = candidatesToImport.map(candidate => {
        const isShortCourse = candidate.category === 'Short Course'
        ...
        return {
          ...
          course: isShortCourse 
            ? selectedCoursesForImport[0] // Use first selected course as primary
            : candidate.role === 'Nanny' 
              ? 'Professional Nanny Training Program'
              : 'Professional House Manager Training Program',
          cohort_id: selectedCohortForImport,
          training_type: isShortCourse ? 'weekend' : '2week',
          enrolled_courses: isShortCourse ? selectedCoursesForImport : null,
          created_by: staff?.name || user?.email || 'Unknown',
          updated_by: staff?.name || user?.email || 'Unknown'
        }
      })
      ...
      const candidateUpdates = candidatesToImport.map(candidate => ({
        ...
        category: candidate.category // Keep existing category
      }))
      ...
      for (const candidate of candidatesToImport) {
        await ActivityLogger.logCreate(
          ...
          'niche_training',
          '',
          `Imported ${candidate.name} as ${candidate.category} trainee`,
          staff?.name || user?.email || 'Unknown'
        )
      }
      ...
      setSelectedCohortForImport('')
      setSelectedCoursesForImport(['Skills Training'])
      loadTrainingRecords()
    } catch (error: any) {
      ...
      <div className="flex flex-col gap-3">
        <div className="flex gap-3 items-center">
          ...
          >
            ...
            {[...cohorts].sort((a, b) => a.cohort_number - b.cohort_number).map(cohort => (
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                <label className="block text-sm font-medium text-gray-700 mb-2">Select Cohort for Import</label>
                <select
                  value={selectedCohortForImport}
                  onChange={(e) => setSelectedCohortForImport(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-nestalk-primary focus:border-transparent"
                >
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status & Category</th>
                    </tr>
                  </thead>
                        ...
                        <td className="px-4 py-3 text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              candidate.category === '2-Week Flagship' 
