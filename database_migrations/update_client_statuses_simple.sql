...
ALTER TABLE clients ADD CONSTRAINT clients_status_check 
  CHECK (status IN (
    -- Lead statuses
    'Pending', 'Lost',
    -- Client statuses (after PAF paid)
    'Client - Active', 'Client - Reviewing Profiles', 'Client - Profile Sent but no response', 
    'Client - Conducting Trials', 'Client - Payment Pending', 'Client - Won', 'Client - Lost',
    -- Legacy statuses (keep for existing data)
    'Active', 'Reviewing Profiles', 'Profile Sent but no response', 'Conducting Trials', 'Payment Pending', 'Won',
...
-- Update existing statuses to new simplified format (optional - can be done gradually)
-- This preserves all existing data while allowing new simplified statuses
...
RETURNS boolean AS $$
BEGIN
  RETURN client_status LIKE 'Client -%' 
         OR client_status IN ('Active', 'Reviewing Profiles', 'Profile Sent but no response', 
